...
classify_above_treeline <- function(coords, gstRaster, gslRaster, gstTreshold = 6.4, gslTreshold = 94) {
  ...
  coords <- cbind(coords, "growingSeasonTemperature" = raster::extract(gstRaster, coords[, 1:2]))
  coords <- cbind(coords, "growingSeasonLength" = raster::extract(gslRaster, coords[, 1:2]))
