...
#' @param gstRaster Raster layer that contains the growing season temperature.
#' @param gslRaster Raster layer that contains the growing season length.
#' @param elevationRaster Raster layer that contains a digital elevation model.
...
#' gst <- raster::raster(gstURL)
#' gsl <- raster::raster(gslURL)
#'
...
#' gmted2010Part <- raster::raster(gmted2010URL)
#'
#' #Check a few points
...
distance_to_treeline <- function(lon, lat, gstRaster, gslRaster, elevationRaster, elevation, pointDf = pointsAboveTreeline, gridSize = 10, gridStepSize = 0.0025,
  ...
  if (class(gstRaster)[1] != "RasterLayer") {stop("gstRaster must be from class RasterLayer")}
  if (class(gslRaster)[1] != "RasterLayer") {stop("gslRaster must be from class RasterLayer")}
  if (class(elevationRaster)[1] != "RasterLayer") {stop("elevationRaster must be from class RasterLayer")}
