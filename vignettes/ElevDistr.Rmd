...
If a digital elevation model of the whole globe is needed the [GTOPO30.tif](https://github.com/LivioBaetscher/ElevDistr/inst/extdata/GTOPO30.tif) can be down loaded from my GitHub account. GTOPO30 courtesy of the U.S. Geological Survey. All the layers can be importer by using:
```{r eval = FALSE}
temp <- raser::raster("your_raster_layer.tif")
```
...
gtopo30 <- raster::raster(installPath)
```
...
gst <- raster::raster(file = "your_gst_layer.tif")
gsl <- raster::raster(file = "your_gsl_layer.tif")
